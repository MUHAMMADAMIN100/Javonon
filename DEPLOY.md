...
### Уникальные индексы и `db push`

Добавление `@@unique` — не удаление, но `db push` всё равно считает его
потенциально разрушительным: статически он не знает, есть ли в таблице
конфликтующие строки, и требует `--accept-data-loss`. Возвращать флаг ради
одного индекса нельзя (он снимает защиту и от настоящих `DROP COLUMN`), поэтому
такие индексы создаются явным SQL в отдельном скрипте ДО `db push`:

- `prisma/ensure-commission-unique.ts` — `Commission(partnerId, studentId)`
  («партнёру платим один раз за клиента»);
- `prisma/ensure-salary-unique.ts` — `SalaryRecord(userId, periodStart)`
  (гард от двойного начисления зарплаты и комиссии менеджера за один период).

Оба идемпотентны (`IF NOT EXISTS`) и оба безопасны: если конфликтующие строки
уже есть, индекс НЕ создаётся, а в лог уходит список конкретных пар. Тогда
`db push` упрётся в то же предупреждение, но с понятной причиной в логе — надо
разобрать дубли руками, а не подавлять флагом. Для зарплаты дубль пары
`(userId, periodStart)` означает, что за один и тот же период зарплата начислена
дважды: лишнюю DRAFT-запись удалить, PAID — разбирать вместе с расходной
SALARY-транзакцией, которую под неё уже нарезали.
