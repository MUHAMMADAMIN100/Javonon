 ...
 *
 * ЧЕГО БОТ НЕ ДЕЛАЕТ: он НЕ создаёт Application. Лид-форму бот не ведёт
 * (ни ФИО, ни телефон, ни страну не спрашивает) — вместо этого кнопка
 * «🚀 Купить» уводит человека на форму лендинга ('/?ref=CODE#apply'),
 * и заявку создаёт уже она через POST /applications/public. Точка захвата
 * лида ровно одна, дублирующего пути через бота нет; всё, что бот пишет
 * в БД, — это BotSession + реферальная атрибуция (source: 'BOT').
 * Не дописывай сюда «бот присылает direction/секондари-контакты» — этого
 * нет, и на таких утверждениях уже один раз построили мёртвую ветку
 * в ApplicationsService.create().
 *
