...
export class SettingsController {
  ...
  //
  // Сетка задана в коде (common/bonus-bands.ts): flat-по-полосе, границы
  // включительные. GET отдаёт её проекцию (readOnly: true) — CRM показывает
  // действующие полосы. POST/PATCH/DELETE оставлены только для обратной
  // совместимости маршрутов и отвечают 400: править ставки через API
  // больше нельзя (иначе снова два источника правды). Таблица BonusTier и
  // её строки в БД сохранены, но не используются.

  /** Действующие полосы комиссии — read-only проекция константы. */
  @Get('bonus-tiers')
